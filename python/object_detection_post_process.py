...
def extract_detections(image: np.ndarray, detections: list, config_data) -> dict:
    ...
    # For YOLOv11 with HailoRT post-processing, detections is a list of arrays
    # Each array corresponds to a class, and contains detections in format [x1, y1, x2, y2, score]
    all_detections = []

    for class_id, detection_array in enumerate(detections):
        ...
        # Handle both single detection and multiple detections per class
        if detection_array.ndim == 1:
            # Single detection: [x1, y1, x2, y2, score]
            if len(detection_array) >= 5:
                bbox = detection_array[:4]
                ...
                if score >= score_threshold:
                    ...
                    all_detections.append((score, class_id, pixel_bbox))
        elif detection_array.ndim == 2:
            # Multiple detections: [[x1, y1, x2, y2, score], ...]
